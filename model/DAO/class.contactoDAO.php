...
class contactoDAO extends dataSource implements IContacto {
    ...
    public function search($contacto) {
    ...
    public function selectById($id) {
        $sql = 'SELECT con_id, con_foto, art_nombre, con_apellido, con_telefono, con_correo FROM contacto WHERE con_id = :id';
        $params = array(
            ':id' => $id
        );
        return $this->query($sql, $params);
    ...
    public function update(\contacto $contacto) {
        $sql = 'UPDATE contacto SET con_foto = :foto, con_nombre = :nombre, con_apellido = :apellido, :  WHERE con_id = :id';
        $params = array(
            ':foto' => $contacto->getFoto(),
            ':nombre' => $contacto->getNombre(),
            ':apellido' => $contacto->getApellido(),
